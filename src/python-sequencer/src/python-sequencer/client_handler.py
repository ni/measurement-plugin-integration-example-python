...
import ni_measurement_plugin_sdk_generator
...
def analyze_functions_and_parameters(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and not node.name.startswith("_") and not node.decorator_list:
...
def configure_init_file(client_module_directory: pathlib.Path, list_of_class_names: List[str], list_of_module_names: List[str]):
...
def create_new_sequence_file(
    ...
    sequence_file_content.append('pin_map_path = r"path\\to\\pinmap\\file"  # update your pinmap path here\n\n')
...
def write_sequence_file(list_of_client_directories: List[pathlib.Path], user_directory: pathlib.Path, list_of_class_names: List[str]):
    methods, _ = analyze_functions_and_parameters(list_of_client_directories[0]) # passing the first client as all clients has the same methods
    pinmap_methods = [
        f"{func}" for func in methods if "pin" in func.lower()
    ]
    create_new_sequence_file(file_path=user_directory / "custom_sequencer.py", instance_names=[client.lower() for client in list_of_class_names], callables=pinmap_methods)
...
def clean_up(user_directory):
    """Deletes all files in the Clients directory and removes custom_sequencer.py."""
    ...
    delete_file(sequencer_path)  # Delete the file
...
def create_client(target_path: Optional[pathlib.Path] = None) -> None:
    ...
    for measurement in available_measurement_services:
        print(f"{measurement.service_class}\n")
    ...
    configure_init_file(client_module_directory=client_module_directory, 
                        list_of_class_names=list_of_class_names, 
                        list_of_module_names=list_of_module_names)
    
    write_sequence_file(list_of_class_names=list_of_class_names, 
