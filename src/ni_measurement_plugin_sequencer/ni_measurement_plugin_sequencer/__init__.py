...
def create_sequence(directory_out: pathlib.Path) -> None:
    """The Python sequencer is a CLI tool that uses the ni-measurement-plugin-client-generator\
        to generate clients and creates a getting-started sequence.

    The `directory_out` argument is a mandatory path to the directory where \
        sequence files are stored.
    """
    try:
